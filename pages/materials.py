...
Includes: table with USE button, radar comparison, bubble scatter.
"""

from dash import html, dcc
from engine.materials_db import MATERIALS, MATERIAL_CATEGORIES
from visualization.gauges import (
    C_TEXT, C_TEXT_DIM, C_CYAN, FONT_FAMILY, TRANSPARENT
)

CARD_BG = "#111820"
CARD_BORDER = "#1E2A38"

# Muted dark-bg-friendly chart palette
CH_TEAL = "#3D8B8B"       # feasible / positive
CH_TEAL_DIM = "rgba(61,139,139,0.15)"
CH_CORAL = "#8B5A5A"      # not feasible / negative
CH_CORAL_DIM = "rgba(139,90,90,0.15)"
CH_SAND = "#8B7D5A"       # caution / neutral
CH_BLUE = "#4A7A9B"       # info / accent
CH_PURPLE = "#7A6A9B"     # secondary accent
...
def make_material_row(mat, eval_data):
    """Build a single material row for the comparison table."""
    ...
    return html.Tr(
        ...
        children=[
            html.Td(mat.name, style={"padding": "8px 6px", "color": C_TEXT,
                     "fontSize": "12px", "fontWeight": "600"}),
            html.Td(mat.category, style={"padding": "8px 6px", "color": C_TEXT_DIM,
                     "fontSize": "11px"}),
            html.Td(f"{mat.density_kg_m3:.0f}", style={"padding": "8px 6px",
                     "color": C_TEXT, "fontSize": "12px", "textAlign": "right"}),
            ...
            html.Td(f"{eval_data['brs_rpm']:.0f}", style={"padding": "8px 6px",
                     "color": CH_BLUE, "fontSize": "12px", "textAlign": "right"}),
            html.Td(f"{eval_data['total_stress_MPa']:.1f}", style={"padding": "8px 6px",
                     "color": C_TEXT, "fontSize": "12px", "textAlign": "right"}),
            html.Td(sf_text, style={"padding": "8px 6px",
                     "color": CH_TEAL if struct_ok else CH_CORAL,
                     "fontSize": "12px", "textAlign": "right", "fontWeight": "600"}),
            html.Td(status_text, style={"padding": "8px 6px", "color": status_color,
                     "fontSize": "11px", "fontWeight": "700", "textAlign": "center"}),
            html.Td(
                html.Button(
                    "USE",
                    id={"type": "use-material-btn", "index": mat.name},
                    n_clicks=0,
                    style={
                        "backgroundColor": "#1A2535",
                        "color": C_CYAN, "border": f"1px solid {C_CYAN}",
